                    ...
                    columnDefs : [ // 각 컬럼들에 대한 커스터마이징
                        ...
                        {orderable: false, targets :[1,3,4,5,6,7,8,9]},
                        {
                            ...
                            createdCell : function(td, cellData, rowData, row, col) {
                                if(cellData == 'X') {
                                    $(td).css('color', 'red');
                                    console.log($(this).DataTable().settings()[0].aoColumns[col]);
                                    console.log(rowData);
                                    console.log(row);
                                    console.log(col);
                        ...
                        {targets : 10, visible : false}, // type - db, doc
                        {targets : 11, visible : false}, // check
                        {targets : 12, visible : false} // wrongColumns
                        // {responsivePriority : -3 , targets: 0}, // responsivePriority 숫자가 클수록 먼저 사라짐. 음수는 사라지지 않게
                        // {responsivePriority : -2 , targets: 1}
                    ],
                    createdRow : function(row, data, dataIndex, cells) { // 불일치한 컬럼 빨간색
                        if(data.type == 'doc' && data.match == 'X' && data.check == 'O') {
                            // console.log(data.wrongColumns);
                            // console.log(data[data.wrongColumns[0]]);
                        }
    
                        if(data.type == 'doc' && data.match == 'X' && data.check == 'X') { // 아예 없는 경우
                            $(row).addClass('red');
                        }
                    },
                    language:
