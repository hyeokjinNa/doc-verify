...
//				"precision", "scale", "notNull", "pk"};
		...
		for (TableDTO firstTable : firstTableList) {
			...
			for (TableDTO secondTable : secondTableList) {
				...
				if (StringUtils.equalsIgnoreCase(firstRow.get(10), secondRow.get(10))
						&& StringUtils.equalsIgnoreCase(firstRow.get(1), secondRow.get(1))) {
					...
					for (int col = 2; col < tableKeys.length; col++) {
						...
						if (!StringUtils.equalsIgnoreCase(firstRow.get(col), secondRow.get(col))) {
		...
		for (ParameterDTO parameterDto : parameterDtoList) {
			...
			for (TableDTO tableDto : tableDtoList) {
				if (!tableNameMap.containsKey(tableDto.getTableName())) {
					tableNameMap.put(tableDto.getDbTableName() + ":" + tableDto.getPhysicalName(),
							tableDto.getSchema());
				}
			}
			...
			for (String tableName : tableNameMap.keySet()) {
				...
				TableDTO dto = new TableDTO.TableBuilder().schema(tableNameMap.get(tableName))
						.dbTableName(tableName.split(":")[0]).physicalName(tableName.split(":")[1]).build();
